...
''' Filter data at 3 Hz to remove any high frequency components'''
analog_frequency = 1000
cut_off = 2
order = 4 # Weyand (2017)
...
def get_toe_off_left(time: np.array, az_filt_l: np.array):
	...
	for step in range(2000,len(az_filt_l), 2000):
		...
		for i in range(len(temp)):
			...
			if delta <= (1/(1 - 1/1.25)) and maxima_value > 0:
				sig_maxs_ind.append(temp[i])
		...
		try:
			...
			if m < 350:
				...
				try:
					...
					max_maxima = az_filt_l[max_maxima_ind] / 2 # Select a mid range value, should not effect selections
	...
	TO_list = []
	TO_min = np.quantile(np.array(az_filt_l[temp_sig]), 0.75) # Arbitrary value
		...
		if maxima in no_add:
			...
			if (i == len(temp_sig) - 1):
				...
				if (dist >= 400) and (m < m_av + 150):
					if az_filt_l[maxima] > TO_min / 2.5:

						sig_maxs_ind.append(maxima)
						TO_list.append(az_filt_l[maxima])
						TO_min = min(TO_list)
						TO_mean = np.mean(np.array(TO_list))
