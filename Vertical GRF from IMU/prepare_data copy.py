...
def prepare_data(data: np.ndarray, dataset: np.array, HS_TO: list, dataset_type: str, sample_length: int):
	...
	if dataset_type == 'training' or dataset_type == 'testing':
		...
		start_period = nsamples * sample_length
		...
		temp = []
		for i in range(len(new_a_l)):
			temp.append(np.concatenate(((new_a_l[i])[start_period:], np.array([new_a_l[i,-1]] * (sample_length - remaining_timesteps)))))
		
		for i in range(len(new_a_r)):
			temp.append(np.concatenate(((new_a_r[i])[start_period:], np.array([new_a_r[i,-1]] * (sample_length - remaining_timesteps)))))
